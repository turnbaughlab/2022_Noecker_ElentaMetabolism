...
on_off_plot3 
fwrite(fva_data[!grepl("Exchange", Subsystem) & Rxn != "bio1" & Subsystem %in% top_top_sub & !is.na(Subsystem), list(Subsystem, Rxn, On)][order(Subsystem)], file = "figure_source_data/figure3B_topSubsysRxns.tsv", quote=F, row.names = F, sep = "\t")
...
single_robustness_data[,Compound:="Acetate"]
single_robustness_data_arg[,Compound:="Arginine"]
single_robustness_data_all <- rbind(single_robustness_data, single_robustness_data_arg, use.names = F)
fwrite(single_robustness_data_all[,list(Compound, Model, CompoundFlux = acetate_fluxes, GrowthRate = acetate_growthRates)], file = "figure_source_data/figure3C_CODHrobustness.tsv", quote=F, row.names = F, sep = "\t")

################ Growth LOO comparison
#loo_results <- fread(paste0(growth_dir, "suppTable_loo_growth_summary.tsv")) # Something is wrong with the Uracil growth results
loo_results <- data.table(read_xlsx("supp_tables/TableS2_growthParams.xlsx"))
...
loo_results[,GrowthBinary:=ifelse(FracR > 0.01 & FracK > 0.05 & FractionAUC_e > 0.04, 1, 0)]
...
single_loo_results[!is.na(GrowthBinary)][SimulationGrowthRate > 0 & GrowthBinary==0]
# Get a p-value
...
data_save <- single_loo_results[,list(CompoundID, MetName, Category, SimulationGrowthRate, ExperimentalAUCe, GrowthBinary, FBAGrowthBinary = SimulationGrowthRate > 0)]
fwrite(data_save, file = "figure_source_data/figure3DE_LOO_FBA_comparison.pdf", sep = "\t", row.names = F, quote = F)
...
secretion_plot
exch_data[,Met_Log2FC:=]
fwrite(exch_data[MetOutcome != "No data", list(Rxn, MetName, FVA_LB, FVA_UB, pFBA, MetOutcome, FVAPred, Met_Log2FC=V1)][order(pFBA)], file = "figure_source_data/figure3F_FVAsecretionUptake.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(curated_mod_summary[,list(Type, Feature, Number)], file = "figure_source_data/figure3A_rxnCounts.tsv")
...
source_dat <- strain_genes[!is.na(pFBAFlux) & !is.na(meanBaselineTPM), list(Genes, locus_tag, Rxn, Subsystem, pFBAFlux=pFBAFlux2, pFBA_FluxQuantile=fluxQuantiles, meanBaselineTPM)]
fwrite(source_dat, file = "figure_source_data/figure3G_fluxRNAcompare.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(ko_results_all2[,list(delRxn, RxnName=Var2, RxnSubsystem = Var3, grRatio, grRateKO, HasEffect = HasEffect2, EDM_grRatio = DM1b_grRatio, EDM_grRateKO = DM1b_grRateKO, EDM_HasEffect = DM1b_HasEffect2)], file = "figure_source_data/figureS7A_subsysKOs.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(strain_genes[,list(Rxn, Gene, locus_tag, ClusterID, NumStrainsOutOf31 = NumStrains, grRatio, grRateKO, HasEffect = HasEffect2)], file = "figure_source_data/figureS7B_strainConservationKOs.tsv", 
       quote=F, row.names = F, sep = "\t")

```


Figure S6 - limiting biomass components
```{r}
##
rm(list = ls())
library(data.table)
...
fwrite(combined_dat[,list(Compound, ModelValue, NewBiomassValue=Var1, FBAGrowthRate=growth_rate_results)], file = "figure_source_data/figureS6_biomassCoefs.tsv", quote=F, row.names = F, sep = "\t")
