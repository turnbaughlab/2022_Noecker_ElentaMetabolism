...
processed_data <- fread("processedDatasets/time_course_all_data_clustered_features_test.csv")
summary_data <- fread("processedDatasets/timecourse_mean_summaries_clustered_features_test.csv")

summary_data <- summary_data[!IDUniq %in% processed_data[Duplicate==1, unique(IDUniq)]]
...
fwrite(growth2_sub[,list(Strain, Time, AcConc, Replicate, OD600)], file = "figure_source_data/figure1d_growth_data.tsv", quote=F, row.names = F, sep = "\t")
...
processed_data[,CF_superclass2:=ifelse(CF_superclass %in% class_counts[V1 < 5, CF_superclass], "Other", CF_superclass)]


fwrite(processed_data[Strain == 2243 & AcConc == 1, list(Sample, Strain, AcConc, Replicate, IDUniq, IonMode, MSI, AvgRt, AvgMZ, MetName, INCHIKEY, Duplicate, value, log10value, MinFeatValue, CF_superclass2)], file = "figure_source_data/figure1_met_data.tsv", quote=F, row.names = F, sep = "\t")
...
summary_data[!is.na(p.value) & TimePoint==6 & AcConc==1 & Strain == "2243", fisher.test(table(MSI != "", PresentInCtrls2 == 1 & Present == 1))]
summary_data[!is.na(p.value) & TimePoint==6 & AcConc==1 & Strain == "2243", fisher.test(table(MSI == 1, PresentInCtrls2 == 1 & Present == 1))]
summary_data[!is.na(p.value) & TimePoint==6 & AcConc==1 & Strain == "2243", fisher.test(table(MSI == 2, PresentInCtrls2 == 1 & Present == 1))]
summary_data[!is.na(p.value) & TimePoint==6 & AcConc==1 & Strain == "2243", fisher.test(table(MSI == 3, PresentInCtrls2 == 1 & Present == 1))]
summary_data[!is.na(p.value) & TimePoint==6 & AcConc==1 & Strain == "2243", fisher.test(table(MSI == 4, PresentInCtrls2 == 1 & Present == 1))]
...
```{r fig_s1}
####### Figure S1
#version with dups removed: c(29, 31, 40, 47, 56, 61, 62, 66, 91, 97, 98, 100, 101, 102, 104, 106, 112, 115, 122, 123, 124, 128)
summary_data <- fread("processedDatasets/timecourse_mean_summaries_clustered_features_test.csv")
## Need pos and neg including duplicates here
...
processed_data[,MediaComponent:=ifelse(IDUniq %in% summary_data[MediaComponent==1, IDUniq], 1, 0)]
fwrite(processed_data[Strain == 2243 & AcConc == 1, list(Sample, Strain, AcConc, Replicate, IDUniq, IonMode, MSI, AvgRt, AvgMZ, MetName, INCHIKEY, MediaComponent, Duplicate, value, log10value, MinFeatValue, CF_superclass2)], file = "figure_source_data/figure1_figureS1_met_data.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(counts_all, file = "figure_source_data/figureS1c_datasetCompare.tsv", quote=F, row.names = F, sep = "\t")
# blue = HSL 7, 2, 255
# red = rgb 255, 4, 1
...
fwrite(corrected_data_all[,list(Experiment, Condition, BaseDM1Condition, Row, Column, well, TimeHr=Time2, NormalizedValue = NewValue)], 
       file = "figure_source_data/figureS2_growthData.tsv", quote=F, row.names = F, sep = "\t")
...
model_fits_compare_good2b[, LOOCategory3:=ifelse(LOOCategory2 %in% c("Others", "Amino acids"), "Amino acids &\nother carbon sources", LOOCategory2)]

fwrite(model_fits_compare_good2b[,list(LOOCategory3, LOO, Sample, Experiment, variable, value)], file = "figure_source_data/figureS2_growthDataParameters.tsv")
...
base_models_sub <- base_models[BadFit==0 & Strain == "2243" & Experiment %in% model_fits_compare_good2b[,Experiment] & !Experiment %in% bad_base]
...
diff_results[,table(r_padj < 0.2|k_padj < 0.2|tmid_padj < 0.2|auc_padj < 0.2)]
diff_results[r_padj < 0.2|k_padj < 0.2|tmid_padj < 0.2|auc_padj < 0.2]
diff_results[r_padj < 0.1]
...
#### Presentation plots
ggplot(average_dat3[LOOGroup=="Arg" & ColorCol=="EDM1"], aes(x = time, ymin = V1-V2, ymax = V1+V2, group = Condition2), fill = "gray")  + geom_ribbon(alpha = 0.4) + geom_line(aes(y = V1), color = "gray") + ylab("E. lenta abundance") + xlab("Time (hr)")+ theme_bw()+ theme(strip.background= element_blank(), panel.grid = element_blank())
ggplot(average_dat3[LOOGroup %in% c("Arg", "Acetate")], aes(x = time, ymin = V1-V2, ymax = V1+V2, group = Condition2, fill = ifelse(ColorCol == "EDM1", T, F)))  + geom_ribbon(alpha = 0.4) + geom_line(aes(y = V1, color = ifelse(ColorCol == "EDM1", T, F))) + ylab("E. lenta abundance") + xlab("Time (hr)")+ scale_color_manual(values = c("#FB8072", "grey50"), name = "Removed compound class") + scale_fill_manual(values = c("#FB8072", "grey50"), name = "Removed compound class") + facet_wrap(~factor(ifelse(LOOGroup=="Arg", "Arginine", "Acetate"), levels=c("Arginine", "Acetate"))) + theme_bw()+ theme(strip.background= element_blank(), panel.grid = element_blank())
...
## carboxylic acids exp
ggplot(all_growth_data[Experiment == "AddCarboxylicAcids"], aes(x = Time2, y = NewValue, color = Condition2)) + geom_line(aes(group = well))
fwrite(all_growth_data[Experiment == "AddCarboxylicAcids", list(TimeHr = Time2, Condition, well, NormalizedOD600 = NewValue)], file = "figure_source_data/figure2B_carboxylicAcids_growth.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(processed_data[Strain %in% c("2243",  "AB8", "Val") & AcConc %in% c(0,1, 10) & IDUniq %in% variable_feat_order_sub & Duplicate == 0, list(IDUniq, IonMode, AlignmentID, AvgRt, AvgMZ, MetName, Sample, Strain, AcConc, TimePoint, Time, Replicate, SampleType, value, log10value, ZScoreLog)], file = "figure_source_data/figure2C_figureS3C_acetate_met_heatmap.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(growth2[,list(Time, Strain, AcConc, Replicate, OD600)], file = "figure_source_data/figureS3B_acetateStrainsGrowth.tsv", quote=F, row.names = F, sep = "\t")
...
raw_data_save <- lower_level_data_long[TimePoint=="TP5" & CompID %in% c(top_comps2, top_comps_ab8, top_comps_val) & AcetateGroup %in% c("1L", "10L") & AreaFrac != 0, list(CompID, Formula, Tags.y, AnnotationMolWt, RT, NumLabeledC, Sample, Strain, AcetateGroup, Replicate, AreaFrac)]
fwrite(raw_data_save, file = "figure_source_data/figure2E_S4CD_intracellularAcetateMets.tsv", quote=F, row.names = F, sep = "\t")
...
# Figure 2/Figure S4 - acetate SIRM experiment, extracellular data
...
### Compare with unlabeled data
comps_to_compare <- summary_tab_ex[Labeled > 50000 & Labeled/Unlabeled > 0.1, sort(unique(CompID))]
processed_data <- fread("processedDatasets/timecourse_all_data.csv")
summary_data <- fread("processedDatasets/timecourse_mean_summaries.csv")
growth_file = "../TimeCourse_take2_2020-8-17/ODdata.xlsx"
time_pts = read_xlsx(growth_file, sheet = 2)
...
## Which mets are significantly diff with acetate 0 vs 1?
load(paste0(outdir, "santaR_acetate0v1results.rda"))
all_pvals <- sapply(test1, function(x){ return(x$general$pval.dist)})
...
santar_summary_ac0v1[,IDUniq:=gsub("^X", "", IDUniq)]
setnames(santar_summary_ac0v1, names(santar_summary_ac0v1)[2:6], paste0("Ac0v1_", names(santar_summary_ac0v1)[2:6]))
santar_summary_ac0v1 <- merge(santar_summary_ac0v1, unique(processed_data[,list(IDUniq, MetName)]), by = "IDUniq", all.x = T)
santar_sub <- santar_summary_ac0v1[Ac0v1_dist_BH < 0.25 & !grepl("known", MetName)]
table(sort(tolower(gsub("_[A-Za-z]+", "", gsub("-l-", "", comps_to_compare)))) %in% tolower(santar_sub[,MetName]))
...
raw_dat2ex <- low_level_data[Sample2 != "c_10L_TP8_1" & !CompID %in% dup_comps & (CompID %in% top_comps2ex_sub|CompID %in% top_comps2ex_otherspec)& Strain != "c" & AcetateGroup %in% c("10L") & !grepl("ASCORBIC", CompID) & value != 0, list(CompID, FeatID, MolWt, Formula, MSI, NumLabeledC, NumLabeledC2, AcetateGroup, Strain, Time, Sample, value, AreaFrac)]
fwrite(raw_dat2ex, file = "figure_source_data/figureS4B_extracellular_labels.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(labeled_compounds_count[,list(TimePoint, Time, Strain, AcetateGroup = AcetateGroup2, Replicate, NumberLabeledComps=V1)], file = "figure_source_data/figure2D_numLabeledComps.tsv", quote=F, row.names = F, sep = "\t")
...
fwrite(od_data, file = paste0("figure_source_data/figureS4A_SILAcetateGrowth.tsv"), quote=F, row.names = F, sep = "\t")
...
# Figure 2A-B: Acetate related growth experiments
```{r acetate_growth}
outdir <- "figure2/"
dir.create(outdir)
...
source("scripts/growth_curve_functions.R")
#### Replot of acetate
datafile <- "../SILAcetateTimeCourse/DM1Ac_SilPretest_2020-10-30.txt"
metadata_file <- "../SILAcetateTimeCourse/mediaPlan_wTestLayout_updated2.xlsx"

# Set up metadata
layout_info <- data.table(read_xlsx(metadata_file, sheet = 3))
layout_info <- layout_info[1:16,1:13, with=F]
...
layout_info <- dcast(layout_info[!is.na(Row)], Row+Column~Type, value.var = "value")
layout_info[,well:=paste0(Row, Column)]
layout_info[,Strain:=case_when(
  grepl("_c$", Media) ~ "c",
  Strain == 2243 ~ "2243",
  Strain == "Val" ~ "Valencia",
  Strain == "AB8" ~ "AB8n2"
)]
layout_info <- layout_info[!is.na(Media) & !is.na(Strain)]

layout_info[,AcetateConc:=gsub("DM1Ac", "", Media)]
layout_info[,Media:=gsub("_c$", "", Media)]
#layout_info[well ]

layout_info[,Condition:=paste0(Media, Row)]

blank_wells1 = layout_info[Strain=="c", well]
names(blank_wells1) = layout_info[Strain=="c", Condition]
...
growth_data_long <- growth_data_long[!is.na(value)] #cut off readings

# Normalize based on blank wells
corrected_data <- correct_growth_data_custom(growth_data_long, blank_wells = blank_wells1, 
                                             method = "blankTimeMatch", wide_form = F)#, ctrlMaxTime = 40, ctrlMinTime = 30)
# Ideal - do time matched for part 1 and median blank (with cutoff) for part 2
...
model_fits <- fit_growthcurver_models(growth_data_norm, indiv_fits = F, trim_after = 45)


plot1 = plot_growth_curves(growth_data_norm = growth_data_norm[,c("time", layout_info[Strain == "2243", well]), with = F], 
                           group_metadata = layout_info[Strain == "2243"], 
                           color_var = "AcetateConc", mean_se = T) + xlim(0, 44) + ylab("Cell density (OD600)") + xlab("Time (hr)") + theme_classic() + scale_fill_brewer(palette = "Set1", name = "Acetate (mM)")# + scale_color_distiller(palette = "Blues", direction = 1) + ylim(0, 0.6)
ggsave(plot1, file = paste0(outdir, "preTest2243Curves.pdf"), width = 4, height = 3)

growth_data_norm1 <- melt(growth_data_norm[,c("time", layout_info[Strain == "2243", well]),with=F], id.var = "time", variable.name = "well", variable.factor = F, value.name = "NormalizedOD600")
growth_data_norm1 <- merge(growth_data_norm1, layout_info[Strain == "2243", list(well, Media, Strain, AcetateConc)], by = "well")
fwrite(growth_data_norm1, file = "figure_source_data/figure2A_growth.tsv", quote=F, row.names = F, sep = "\t")
...
source_dat <- met_data[Compound == "Acetate" & !is.na(Time), list(Sample, Location, Row, Column, AcetateConc, TimePoint, Time, Replicate, LLOQ, ULOQ, mMCorrected = mM-1.05)]
fwrite(source_dat, file = "figure_source_data/figureS3A_targetedAcetate.tsv", quote=F, row.names = F, sep = "\t")
...
raw_data <- hilic_intra[ArgGroup == "HR" & Strain == "2243" & CompID %in% top_id_comps & !grepl("known", Name) & value != 0, list(CompID, FeatID, Name, Formula, MSI, NumLabeledC, Sample, Strain, ArgGroup, TimePoint, Time, Replicate, value, AreaFrac)]
fwrite(raw_data, file = "figure_source_data/figureS5D_argIntracellular.tsv", quote=F, row.names = F, sep = "\t")
...
arg_source_dat <- hilic_extra[ArgGroup == "HR" & Strain == "2243" & FeatID %in% most_abund_iso[V1 != 0 & grepl("Pos$", CompID)][order(V2, decreasing = T)][1:3, paste0(CompID, "_ExRate", V1)]][,list(FeatID, Name, NumLabeledC, Formula, MSI, Sample = Sample2, ArgGroup, Strain, Time, Replicate, AreaFrac)]
fwrite(arg_source_dat[order(FeatID)], file = "figure_source_data/figureS5B_argCitrOrn.tsv", quote=F, row.names = F, sep = "\t")
...
extra_source_data <- hilic_extra[ArgGroup == "HR" & Strain == "2243" & CompID %in% top_comps3 & !grepl("known", Name) & value != 0, list(FeatID, CompID, Name, NumLabeledC, Formula, MSI, Sample = Sample2, ArgGroup, Strain, Time, TimePoint, Replicate, AreaFrac, value)]
fwrite(extra_source_data[order(FeatID)], file = "figure_source_data/figureS5C_argExtracellular.tsv", quote=F, row.names = F, sep = "\t")
...
tot_signal_source_data <- hilic_extra[ArgGroup == "HR" & Strain == "2243" & CompID %in% most_abund_comps_pos & NumLabeledC %in% c(0,5,6) & grepl("Pos$", CompID), list(FeatID, CompID, Name, NumLabeledC, Formula, MSI, Sample = Sample2, ArgGroup, Strain, Time, TimePoint, Replicate, AreaFrac, value)]
fwrite(tot_signal_source_data, file = "figure_source_data/figureS5E_argExtracellularPosAbundFeats.tsv", quote=F, row.names = F, sep = "\t")
...
Figure S5 - Arginine/Citrl growth experiment
```{r}
library(growthcurver)
library(data.table)
library(ggplot2)
...
datadir <- "../GrowthExperiments/2022_06_24_AgmArgCitr_extraComps/"
datafile <- paste0(datadir, "AgmCitrlOrnOtherComps_2243_2022-06-24.txt")
...
blank_wells1 <- blank_wells1[blank_wells1 != "G2"]
# Read in file
growth_data1 = read_growthcurve_file(datafile)
...
trim_after_wells = rep(54, times = length(all_samps))
...
growth_data_norm <- growth_data_norm[,-"G2"]

exp1 <- layout_info[grepl("Agm", Condition), unique(Condition)]
exp2 <- c("EDM1_Arg1Agm0", "Arg0Citrl1", "Arg0Orn1")
exp3 <- layout_info[,unique(Condition)][9:12]

layout_info[,Condition2:=factor(Condition, levels = c("EDM1_Arg1Agm0", "Arg0.5Agm0", "Arg0Agm1", "Arg0Agm0.5", "Arg0Agm0", "Arg0.5Agm0.5",  "Arg0Citrl1", "Arg0Orn1", 
                                                      "Base2_70Ac70Arg", "Base2_NCarbAsp", "Base2_AcetylCarn", "Base2_HexCarn"))]
levels(layout_info$Condition2) <- c("EDM1 (full arginine)", "50% arginine", "Full agmatine", "50% agmatine", "0 arginine, 0 agmatine", "50% arginine, 50% agmatine", 
                                    "Citrulline", "Ornithine", "Base2", "N-Carb-asp", "Acetyl-carnitine", "Hexanoyl-carnitine")
## Bonus figure 6C
plot1 = plot_growth_curves(growth_data_norm = growth_data_norm[,c("time", layout_info[Condition %in% exp1 & Strain == "2243", well]), with=F], 
                           group_metadata = layout_info[Condition %in% exp1], color_var = "Condition2", 
                           facet_var = "Strain", mean_se = T, model_fits = model_fits2) + 
  scale_fill_manual(values = c(brewer.pal(10, "Paired")[c(2,1,6,5, 9, 10)]), name = "Condition") + ylab("E. lenta DSM 2243 abundance\n(normalized OD600)") +
  xlab("Time (hr)")


ggsave(plot1, file ="figure6/arg_agm_doses_plot.pdf", width = 6, height= 3)
fig6c_data <- melt(growth_data_norm[,c("time", layout_info[Condition %in% exp1 & Strain == "2243", well]), with=F], id.var = "time", variable.name = "well", variable.factor =F, value.name = "NormalizedOD600")
fig6c_data <- merge(fig6c_data, layout_info[Condition %in% exp1, list(well, Strain, Condition2)], by = "well", all = F)

fwrite(fig6c_data[Condition2 %in% c("EDM1 (full arginine)", "Citrulline", "Ornithine"), list(Strain, Condition = Condition2, time, well, NormalizedOD600)], file = "figure_source_data/figureS5A_growthArgCitrlOrn.tsv", quote=F, row.names = F, sep = "\t")

fwrite(fig6c_data[Condition2 %in% c("EDM1 (full arginine)", "50% arginine", "Full agmatine", "50% agmatine", "0 arginine, 0 agmatine", "50% arginine, 50% agmatine"), list(Strain, Condition = Condition2, time, well, NormalizedOD600)], file = "figure_source_data/figure6C_growthArgAgm.tsv", quote=F, row.names = F, sep = "\t")

plot2 = plot_growth_curves(growth_data_norm = growth_data_norm[,c("time", layout_info[Condition %in% exp2 & Strain == "2243", well]), with=F], 
                           group_metadata = layout_info[Condition %in% exp2], color_var = "Condition2", 
                           facet_var = "Strain", mean_se = T, model_fits = model_fits2) + 
  ylab("E. lenta DSM 2243 abundance\n(normalized OD600)") +
  xlab("Time (hr)")
ggsave(plot2, file = paste0(datadir, "arg_orn_citrl_plot.pdf"), width = 5.2, height= 3)
