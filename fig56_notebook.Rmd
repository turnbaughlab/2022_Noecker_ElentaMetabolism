...
source_data_intest <- met_data[IDUniq %in% met_data_means[,IDUniq] & Duplicate == 0, list(IDUniq, AlignmentID, IonMode, AvgMZ, AvgRt, MSI, MetName, Formula, INCHIKEY, CF_superclass,
                                                                                          Sample, Animal=Animal2, SampleType, Group, Cage, value, log10value)]
source_data_serum <- serum_met_data[IDUniq %in% serum_met_data_means[,IDUniq] & Duplicate == 0, list(IDUniq, AlignmentID, IonMode, AvgMZ, AvgRt, MSI, MetName, Formula, INCHIKEY, CF_superclass,
                                                                                          Sample, Animal, SampleType, Group, Cage, value, log10value)]
source_data_serum[,IonMode:=ifelse(IonMode=="pos", "Positive", "Negative")]
source_data <- rbind(source_data_intest, source_data_serum)
fwrite(source_data, file = "figure_source_data/figure5A_6A_S10A_S11_mouseMetData.tsv", quote=F, row.names = F, sep = "\t")
```
...
features_tab_source_data <- features_tab[(grepl("Serum", IDUniq)|grepl("Intest", IDUniq)) & IDUniq %in% c(good_intest_features, good_serum_features), list(IDUniq, `Alignment ID`, Dataset, AvgMZ, AvgRt, NewID2, InStrains=InGL, InTimeCourse, InVitro)]
fwrite(features_tab_source_data, file = "figure_source_data/figureS10B_linkedFeatures.tsv", quote=F, row.names = F, sep = "\t")
...
# Source data
source_data_serum <- serum_data_means[!(is.na(SharedShift) & is.na(SharedShiftTimeSeries)), list(IDUniq, IDUniq2, NewID2, AvgMZ, AvgRt, Adduct, MSI, Formula, Strain, Site, Strains_log2FC, Strains_FDRCorrect, TimeSeries_log2FC, TimeSeries_FDRCorrect, GF_log2FCEstimate= EstimateLog2Scale, GF_FDRCorrect = FDRCorrect, SharedShiftTimeSeries, SharedShiftStrain=SharedShift)]
source_data_intest <- met_data_means[!(is.na(SharedShiftStrain) & is.na(SharedShiftTimeSeries)), list(IDUniq, IDUniq2, NewID2, AvgMZ, AvgRt, Adduct, MSI, Formula, Strain, Site, Strains_log2FC, Strains_FDRCorrect, TimeSeries_log2FC, TimeSeries_FDRCorrect, GF_log2FCEstimate= EstimateLog2Scale, GF_FDRCorrect = FDRCorrect, SharedShiftTimeSeries, SharedShiftStrain)]
source_data <- rbind(source_data_serum, source_data_intest)

fwrite(source_data, file = "figure_source_data/figure5B_figureS10C_mergedAvgEstimates.tsv", quote=F, row.names = F, sep = "\t")
